    ...
    if (draggedWidgetId) {
      ...
      if (widgetExistente) {
        ...
        const novoWidget: Widget = {
          ...
          styles: { color: '#ffffff', fontSize: '16px', fontWeight: 'normal', fontStyle: 'normal', textDecoration: 'none', textAlign: 'center', fontFamily: 'Arial, sans-serif' }
        };
  ...
  const handleLoadState = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        setBackground(loadedState.background || { type: 'gradient', solidColor: '#ffffff', gradientAngle: 135, gradientColors: ['#6a89cc', '#3d3bbe'] });
    ...
    <div className={styles.editorContainer}>
      <div className={styles.controlsPanel}>
        ...
        <div className={styles.controlSection}>
          ...
          {widgets.length > 0 ? (
            widgets.map(widget => (
              <WidgetControls key={widget.id} widget={widget} onUpdate={handleWidgetStyleChange} onContentChange={handleWidgetContentChange} onDelete={handleWidgetDelete} />
            ))
      ...
      <div className={styles.previewPanel}>
        ...
        <div className={styles.cardPreviewWrapper}>
          <Card widgets={widgets} colSizes={colSizes} rowSizes={rowSizes} background={background} overlay={overlay} onDrop={handleDropOnCard} />
        </div>
      </div>
    </div>
  );
